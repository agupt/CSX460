...
```{r}
dat <- dplyr::bind_rows(cars2010, cars2011, cars2012)    # Define 
```
...
```{r}
qplot(dat$FE, dat$EngDispl,xlab="Fuel Economy",ylab = "Engine Displacement") 
```
...
samp  %>% ggplot(aes(x=EngDispl, y=FE)) + geom_point() + geom_hline(yintercept = naive_guess,color="red") + stat_smooth(method = "lm", se=F)
...
y.2010 <- fit.2010$fitted.values
y.2011 <- predict.lm(fit.2010, cars2011)
y.2012 <- predict.lm(fit.2010, cars2012)

rmse.2010 <- rmse(cars2010$FE,fit.2010$fitted.values)
...
```{r}
fit.2010 <- lm(FE ~ EngDispl+NumCyl+VarValveTiming+VarValveLift,data=cars2010)
